...
from election.md_instance import MDInstance, MDProject
...
def get_all_categories(file_content: str) -> list:
    ...
    across ALL of a project's listed categories.
    """
...
def md_parse_pabulib_from_string_random_split(
    ...
    budget_multiplier : float
        Scales the original 1D budget B before it is split across dimensions.
        With multiplier c and d dimensions, the total budget is c*B and each
        dimension limit is c*B/d. Defaults to 1.0 (unchanged behaviour). Note
        this only scales the budget, not the (seeded) random cost splits, so
        seed k at c=2 yields the same cost vectors as seed k at c=1.
    """
    ...
    for row in reader:
        ...
        if str(row[0]).strip().lower() in ["meta", "projects", "votes"]:
            ...
            project_cats = sorted(project_meta["categories"])
            ...
            for j, cat in enumerate(project_cats):
                cost_vec[cat_to_idx[cat]] = int(round(float(total_cost * fractions[j])))
    ...
    # Budget: scale the original total budget by budget_multiplier, then split
    # it evenly across dimensions. With multiplier c and d dimensions, the total
    # budget is c*B and each dimension limit is c*B/d.
    total_budget = (
        str_as_frac(instance.meta["budget"].replace(",", "."))
        * str_as_frac(str(budget_multiplier))
    )
    budget_vec = [total_budget / category_count for _ in category_list]
    instance.budget_limits = budget_vec

    # --- Legal ballot constraints ---
...
def md_parse_pabulib_random_split(
    ...
    budget_multiplier : float
        Scales the original 1D budget before it is split across dimensions
        (see md_parse_pabulib_from_string_random_split). Defaults to 1.0.
    ...
    with open(file_path, "r", newline="", encoding="utf-8-sig") as csvfile:
        instance, profile = md_parse_pabulib_from_string_random_split(
            csvfile.read(), seed=seed, budget_multiplier=budget_multiplier
        )
