...
MD-MES for every L^p projection function (sum, L2, L3, L4, max), audits each
...
from rules.md_mes.md_mes_completion_rule import naive_md_mes_with_completion
...
def run(pb_path: Path, sat_name: str, n_trials: int, output_path: Path,
    ...
    for seed in range(start_seed, start_seed + n_trials):
        ...
        for proj_name, proj_func in PROJECTION_FUNCTIONS:
            outcome = naive_md_mes_with_completion(
...
def main():
    ...
    output_path = REPO_ROOT / "data" / f"mes{suffix}" / args.pb_file.stem / "results.json"
