...
MD-Greedy for every L^p projection function (sum, L2, L3, L4, max), audits each
...
from rules.md_greedy.md_greedy_rule import naive_md_greedy
...
def run(pb_path: Path, sat_name: str, n_trials: int, output_path: Path,
    ...
    for seed in range(start_seed, start_seed + n_trials):
        ...
        # Per-project satisfaction value and approval score, used below to
        # compute the social welfare of each outcome.
        ...
        proj_results = {}
        outcome = naive_md_greedy(
            instance, profile, trial_sat_class,
            [personal_bgt] * dim, None, sat_profile,
        )
        ...
        trial_data["Greedy"] = proj_results
...
def main():
    ...
    output_path = REPO_ROOT / "data" / f"greedy{suffix}" / args.pb_file.stem / "results.json"
