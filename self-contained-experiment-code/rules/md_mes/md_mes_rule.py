...
def make_lp_projection(p):
    """Return an L^p norm projection function for p >= 1.
    p=1 is equivalent to projection_function_sum; p->inf to projection_function_max."""
    def projection(affordability_vector):
        return sum(x**p for x in affordability_vector) ** (1.0 / p)
    return projection
